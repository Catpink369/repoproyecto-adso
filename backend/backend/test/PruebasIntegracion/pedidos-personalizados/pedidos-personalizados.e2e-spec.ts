...
describe('RF-005 — Pedidos Personalizados (integración)', () => {
  ...
  // IDs altos y únicos por corrida → evita choque con el seed (ids 1,4,5,6,7)
  const baseId = Math.floor(sufijo % 1000000) + 100000;
  ...
  beforeAll(async () => {
    ...
    materialTelaB = await prisma.material.create({
      data: {
        id_material: baseId + 1,
        nombre: `Tela Test B ${sufijo}`,
        tipo: 'Tela',
        unidad: 'metro',
    ...
    materialExtra = await prisma.material.create({
      data: {
        id_material: baseId + 2,
        nombre: `Sobresabana Test ${sufijo}`,
        tipo: 'Accesorio',
        unidad: 'unidad',
    ...
    materialSinColores = await prisma.material.create({
      data: {
        id_material: baseId + 3,
        nombre: `Tela Sin Colores Test ${sufijo}`,
        tipo: 'Tela',
        unidad: 'metro',
    ...
    it('CP-001: debe guardar la configuración de ambos lados de un cubrelecho de forma independiente', async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          tamanio: 'Queen',
          materiales: [
            { id_material: materialTelaA.id_material, cantidad: 2 },
            { id_material: materialTelaB.id_material, cantidad: 2 },
          ],
        });
  ...
  describe('RF-005.2 — Calcular precio', () => {
    it('CP-005: el precio total debe sumar exactamente el precio de cada opción seleccionada', async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          tamanio: 'King',
          materiales: [
            { id_material: materialTelaA.id_material, cantidad: 3 },
            { id_material: materialTelaB.id_material, cantidad: 2 },
          ],
        });
    ...
    it('CP-007: llamadas consecutivas con combinaciones distintas deben recalcular el total sin arrastrar el precio anterior', async () => {
      ...
      expect(res2.body.precio_total).toBe(36000);
    });
  });
});