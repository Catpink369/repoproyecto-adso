...
export class PedidosPersonalizadosController {
  ...
  async actualizarDiseno(
    @Param('idDiseno') idDiseno: string,
    @Body() dto: UpdateMaterialDisenoDto,
  ) {
    try {
  ...
  async subirImagenDiseno(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Error al subir la imagen del diseño a Cloudinary',
      );
    }
  }
  ...
  async subirImagenMaterial(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Error al subir imagen del material a Cloudinary',
      );
    }
  }
  ...
  @ApiResponse({
    status: 422,
    description: 'Stock insuficiente para uno o más materiales seleccionados.',
  })
    ...
    try {
      ...
      if (error.message?.includes('Stock') || error.status === 422) {
        throw new UnprocessableEntityException(
          'No hay suficiente stock de los materiales seleccionados.',
        );
      }
      if (error.code === 'P2002') {
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de pedidos personalizados obtenida exitosamente.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de pedidos personalizados del usuario obtenida exitosamente.',
  })
  @ApiResponse({ status: 401, description: 'No autorizado - Token no proporcionado.' })
  @ApiResponse({ status: 403, description: 'Prohibido - permisos insuficientes.' })
  ...
  async findByUsuario(@Param('id_usuario') id_usuario: string) {
    try {
      ...
      throw new InternalServerErrorException(
        'Error al obtener pedidos personalizados del usuario',
      );
    }
  }
}