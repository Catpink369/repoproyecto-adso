...
export class MovimientosService {
  ...
  // NOTA MIGRACIÓN MySQL -> PostgreSQL:
  // DATE_ADD(x, INTERVAL 1 DAY) no existe en Postgres.
  // Se reemplaza por: x::date + INTERVAL '1 day'
  private construirFiltroFecha(campo: string, desde?: string, hasta?: string): Prisma.Sql {
    const condiciones: Prisma.Sql[] = [];
    if (desde) condiciones.push(Prisma.sql`${Prisma.raw(campo)} >= ${desde}::date`);
    if (hasta) condiciones.push(Prisma.sql`${Prisma.raw(campo)} <= ${hasta}::date + INTERVAL '1 day'`);
  ...
  async findAll(query: any) {
    ...
    // NOTA: "Cantidad_m" va entre comillas dobles porque el campo en el
    // schema.prisma tiene mayúscula (Cantidad_m, sin @map). Postgres
    // guarda los nombres de columna respetando mayúsculas/minúsculas
    // solo si van entre comillas; sin comillas, Postgres los interpreta
    // todo en minúsculas y no encuentra la columna real.
  ...
  // MOVIMIENTOS POR DÍA
  // -------------------------------------------------------
  // NOTA MIGRACIÓN: DATE_FORMAT(fecha, '%Y-%m-%d') (MySQL) -> TO_CHAR(fecha, 'YYYY-MM-DD') (Postgres)
  async porDia(desde?: string, hasta?: string) {
    this.validarRangoFechas(desde, hasta);
    const where = this.construirFiltroFecha('fecha_m', desde, hasta);
    return this.prisma.$queryRaw<any[]>`
      SELECT 
        TO_CHAR(fecha_m, 'YYYY-MM-DD') as fecha,
      ...
      FROM movimiento ${where}
      GROUP BY TO_CHAR(fecha_m, 'YYYY-MM-DD')
      ORDER BY fecha
    `;
  }
  ...
  // -------------------------------------------------------
  // NOTA MIGRACIÓN: DATE_ADD(x, INTERVAL 1 DAY) (MySQL) -> x::date + INTERVAL '1 day' (Postgres)
  async topProductos(desde?: string, hasta?: string, limit = 10) {
    this.validarRangoFechas(desde, hasta);
    const condiciones: Prisma.Sql[] = [Prisma.sql`p.estado = true`];
    if (desde) condiciones.push(Prisma.sql`m.fecha_m >= ${desde}::date`);
    if (hasta) condiciones.push(Prisma.sql`m.fecha_m <= ${hasta}::date + INTERVAL '1 day'`);
  ...
  // -------------------------------------------------------
  // NOTA MIGRACIÓN:
  //   DATE_FORMAT(x, '%Y-%m')   (MySQL) -> TO_CHAR(x, 'YYYY-MM')   (Postgres)
  //   DATE_FORMAT(x, '%b %Y')  (MySQL) -> TO_CHAR(x, 'Mon YYYY')   (Postgres)
  //   DATE_SUB(CURDATE(), INTERVAL 12 MONTH) (MySQL) -> CURRENT_DATE - INTERVAL '12 months' (Postgres)
  async resumenMensual() {
    const results = await this.prisma.$queryRaw<any[]>`
      SELECT 
        TO_CHAR(fecha_m, 'YYYY-MM') as mes,
        TO_CHAR(fecha_m, 'Mon YYYY') as mes_nombre,
        SUM(CASE WHEN id_m = 'M-E' THEN "Cantidad_m" ELSE 0 END) as entradas,
        SUM(CASE WHEN id_m = 'M-S' THEN "Cantidad_m" ELSE 0 END) as salidas
      FROM movimiento
      WHERE fecha_m >= CURRENT_DATE - INTERVAL '12 months'
      GROUP BY TO_CHAR(fecha_m, 'YYYY-MM'), TO_CHAR(fecha_m, 'Mon YYYY')
      ORDER BY mes
    `;
  ...
  async findAllMaterial(query: any) {
    ...
    // NOTA: aquí NO hace falta comillas para cantidad_m — el campo en
    // el schema es `cantidad_m` en minúsculas (modelo movimiento_material),
    // así que coincide directo con el nombre real de la columna.
    return this.prisma.$queryRaw<any[]>`
      SELECT
